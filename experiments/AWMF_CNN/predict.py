# -*- coding:utf-8 -*-
# Created by Xu Jiayu at 2019/6/1
import os
os.chdir('../../')
import time
from utils.utils_process.common_utils import seconds_regulate
from experiments.AWMF_CNN.data_process import testGenerator, saveResult
import shutil
from experiments.constant import const
from models.AWMF_CNN import *
...
model = AWMF(img_dim=256, batch_size=4, times=LOSS_TIMES).build_AWMF()
model.load_weights(SAVE_WEIGHTS)
results = model.predict_generator(testGene, PREDICT_NUM, verbose=1)
saveResult(save_path=predict_save_path,mode=const.MODE, npyfile=results, times=LOSS_TIMES, drawCircle_path=DRAWCIRCLE_PATH_SAVE, metrics_path=VISUALIZE_PATH)
