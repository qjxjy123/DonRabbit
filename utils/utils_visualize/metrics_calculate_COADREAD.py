...
def getMaskPoints(path, files):
    ...
    for file in files:
        ...
        _, orig_name = os.path.split(file)
        file_PMask = os.path.join(path, orig_name.replace('_mask.png', '_localMask.png'))
...
def sub_metrics_calculate(confusion_matrix):
    ...
    correct_num = np.sum([confusion_matrix[i, i] for i in range(4)])

    total_precision = np.sum([whole[i] / np.sum(whole) * precision[i] for i in range(4)])
    total_recall = np.sum([whole[i] / np.sum(whole) * recall[i] for i in range(4)])
    total_f1 = np.sum([whole[i] / np.sum(whole) * f1_score[i] for i in range(4)])
